...
test("Change perPage @Crash", async ({ page }) => {
    ...
    expect(await app.mainPage.bugInfo).toContainText(mistakeText);
...
test("Select currency @Crash", async ({ page }) => {
    ...
    await app.productDetails.bugInfo.click();
    expect(await app.productDetails.bugInfo).toContainText(mistakeText);
});
...
test("Choose Product in hot item @Perfomance", async ({ page }) => {
    ...
    expect(await app.productDetails.bugSpinner).toBeVisible();
});
...
test("Check manufacturer link @Functional", async ({ page }) => {
    ...
    await app.manufacturer.bugInfo.click();  
    expect(await app.manufacturer.bugInfo).toContainText(pageError);
});
...
test("Check product description @Content", async ({ page }) => {
    ...
    await app.mainPage.clickByFirstProduct();
    await app.productDetails.productDescription.click();
    expect(await app.productDetails.productDescription).toContainText('Nam nec tellus a odio tincidunt auctor a ornare odio.');
});
...
test("Retrieve password @Crash", async ({ page }) => {
    ...
    expect(await app.retrievePassword.bugInfo).toContainText(mistakeText);
});